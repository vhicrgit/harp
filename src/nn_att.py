...
class MultiMyGlobalAttention(torch.nn.Module):
    def __init__(self, gate_nn_P, gate_nn_pseudo_B, nn=None):
        super(MultiMyGlobalAttention, self).__init__()
        self.glob_P = MyGlobalAttention(gate_nn_P, None)
        self.glob_pseudo_B = MyGlobalAttention(gate_nn_pseudo_B, None)

    def forward(self, x, batch, X_pseudonids=None):
        out_P, node_att_scores_P = self.glob_P(x, batch)
        # out_dict['emb_P'] = out_P
        out_pseudo_B, node_att_scores_pseudo = self.glob_pseudo_B(x, batch, set_zeros_ids=X_pseudonids)
        # out_dict['emb_pseudo_b'] = out_pseudo_B
        out_g = torch.cat((out_P, out_pseudo_B), dim=1)
        return out_g, node_att_scores_pseudo, node_att_scores_P
