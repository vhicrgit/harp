...
class MultiheadTransformer(nn.Module):
    def __init__(self, 
        ...
        self.conv1 = TransformerConv(in_channels, hidden_channels, heads=num_heads, dropout=dropout, edge_dim=edge_dim)
        self.conv2 = TransformerConv(hidden_channels * num_heads, out_channels, heads=1, concat=False, dropout=dropout, edge_dim=edge_dim)
...
class MultiheadTransformer2(nn.Module):
    def __init__(self, 
                 in_channels, 
                 out_channels, 
                 hidden_channels=16, 
...
class MultiHeadGlobalAttention(nn.Module):
    def __init__(self, 
                 out_channels = 64*2, 
                 gate_nn = Sequential(Linear(64, 64), ReLU(), Linear(64, 1)),
                 num_heads=4):
        super(MultiHeadGlobalAttention, self).__init__()
        self.global_attentions = nn.ModuleList([GlobalAttention(gate_nn) for _ in range(num_heads)])
        self.fc = nn.Linear(64 * num_heads, out_channels)

    def forward(self, x, batch):
        attentions = [global_attention(x, batch) for global_attention in self.global_attentions]
...
class Net(nn.Module):
    def __init__(self, in_channels, edge_dim = 0, init_pragma_dict = None, task = FLAGS.task, num_layers = FLAGS.num_layers, D = FLAGS.D, target = FLAGS.target):
        ...
        if FLAGS.gnn_type == 'gat':
            ...
            conv_class = MultiheadTransformer2
        else:
            raise NotImplementedError()
        
        if FLAGS.encode_edge and FLAGS.gnn_type == 'transformer':
            self.conv_first = conv_class(in_channels, D, edge_dim=edge_dim, dropout=FLAGS.dropout, num_heads=1)
        else:
            self.conv_first = conv_class(in_channels, D, num_heads=4)
        ...
        for i in range(num_layers - 1):
            ...
            if FLAGS.encode_edge and FLAGS.gnn_type == 'transformer':
                ...
                conv = conv_class(D, D, num_heads=heads)
        ...
        if FLAGS.multi_attention:
            self.gate_nn = self.node_att_gate_nn(D)
            self.glob = MultiHeadGlobalAttention(gate_nn = self.gate_nn)
